...
////////////////
class Ui {
  displayProducts(productsss) {
    // نشان دادن 8 تا پروداکت در صفحه
    let result = "";
    ...
    cartall.addEventListener("click", (event) => {
      if (event.target.classList.contains("fa-chevron-up")) {
        ...
        const substractedItem = cart.find(
          (c) => c.id === parseInt(subQuantity.id)
        );
        ...
        this.setcartalue(cart);
        Storage.savecart(cart);
        subQuantity.parentNode.previousElementSibling.innerText =
          substractedItem.quantity;
      }
    });
  }
...
fetch("https://sunny-blini-25b676.netlify.app").then((v) => {
  console.log(v);
});
